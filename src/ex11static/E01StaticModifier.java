package ex11static;
/*
 static variacle(정적변수 혹은 메서드)
 - 멤버변수에 static 키워드를 붙이면 정적 변수가 된다.
 -정적변수는 JVM(자바가상머신)에 의해 프로그램이 시작되는 시점에 초기화된다.
 - main() 메서드가 실행되기전에 초기화되므로 main()메서드 실행시 별도의
 인스턴스 생성없이 변수를 사용 할 수 있다.
 
 
 접근방법
 - 클래스 내부
 	: 변수명으로 접근할 수 있디ㅏ. 일반적인 멤버변수와 동일하게 접근한다.
 - 클래스 외부
 	1. 인스턴스 변수를 통해 접근한다(권장사항 아님0
...
class MyStatic {
	...
	/*
	  ...
	  JVM(자바가상머신)이 프로그램을 시작할때 미리 메모리(Method영역)에 로드된다.
	 */
	...
	/*
	 정적 멤버메서드 : 정적 메서드 내부에서는 인스턴스형 멤버에는 접근할 수 없다.
	 정적 멤버만 접근할 수 있다.
	 (* main 메서드에서는 static으로 선언된 메서드만 호출할 수 있었음을
	 	기억하자..)
	 */
