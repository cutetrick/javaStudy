...
class Unit {
	...
	Medic medic;
	//생성자
	public Unit(String naem, int healthPoint) {
		//멤버변수 초기화
		this.name = name;
	...
	class Marine {
		...
		int attackPower;
		/*
		 내부클래스에서는 외부클래스의 모든 멤버에 접근 할 수 있다.
...
public class E01InnerClassMain {
	public static void main(String[] args) {
		...
		/*
		 ...
		 생성해야한다. Marine 은 Unit클래스의 생성자에서 이미 인스턴스
		 생성이 되었으므로 아래와 같이 사용할 수 있다.
		 */
		Unit.Marine marine = new Unit("마린", 50).marine;
		marine.attack();
		/*
		 Medic 경우 Unit의 생성자에서 인스턴스 생성이 되지 않았으므로
		 아래와 같이 new를 통해 생성해야한다. 이때에도 Unit인스턴스를 통해
