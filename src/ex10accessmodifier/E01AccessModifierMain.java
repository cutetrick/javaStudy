...
/*
 이 클래스는 sub패키지에 정의되었으므로 사용을 위해서는 반드시
 임포트해야한다. 
 */
...
public class E01AccessModifierMain {
	...
	public static void main(String[] args) {
		System.out.println("ex10accessmodifier1 객체 생성 및 접근");
		...
		/*
		 private으로 선언된 멤버는 동일한 패키지에 선언하더라도 접슨할 수 없다.
...
//		System.out.println("one.privateVar=" + one.privateVar);
		...
		//멤버메서드도 접근범위는 멤버변수와 동일한다.
//		one.privateMethod();//에러발생
		one.defaultMethod();
		one.publicMethod();
		
		/*
		 동일한 패키지에 선언된 클래스이므로 import없이 인스턴스 생성이
		 가능하다. 
		 */
		...
		/*
		 ...
		 import선언시 접근할 수 있는 것은 public이고, private와 default는
		 접근할 수 없다.
		 */
...
//		two.defaultMethod();//에러발생
		...
		/*
		다른 패키지에 선언된 default클래스이므로 인스턴스 생성이 불가능하다.
