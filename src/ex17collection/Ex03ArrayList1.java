...
public class Ex03ArrayList1 {
	...
	public static void main(String[] args) {
		...
		/*
		 인스턴스저장: add()
		 	add(인스턴스) : 순차적으로 저장하면서 인덱스는 0부터 자동을
		 					부여한다.
...
//		list.add(5, "소방차"); //런타임 에러 발생
		/*
		 저장된 객체수(리스트의 크기) : size()
		 	컬렉션에 저장된 인스턴스의 개수를 반환한다. 배열에서 사용하는
		 	length와 동일한 값을 반환한다.
		 */
	...
	/*
	 방법2: 확장 for문의 경우 인덱스 값이 전체 원소를 반복해주므로 
	...
	/*
	방법3: Iterator 사용
	1. 컬렉션에 저장된 내뇽을 Iterator 객체에게 알려주기 위해
			인스턴스를 생성한다.
	...
	/*
	 포함여부확인
	 	contains(인스턴스) : 특정 인스턴스가 저장되어 있는지 확인할때
	...
	/*
	 ...
	 방법 1: 인덱스로 삭제
	 	remove(인덱스): 인덱스를 통해 삭제를 진행한 후 삭제가 
	 		완료되면 해당 인스턴스를 반환한다. 삭제후에는 인덱스가
	 		자동으로 부여된다. 삭제 성공시 해당 인스턴스의 참조값을
	 		 반환하므로 삭제 즉시 확인할 수 있다.
	 */
	...
	/*
	방법2: indexOf()를 통해 해당 인스턴스를 찾은 후 삭제한다.
		인덱스를 찾는것을 제외하면 방법1과 동일하다.
	*/
	...
	/*
	 ...
	 삭제에 성공했을 때 true 를 반환한다. 즉 boolean 타입의
	 반환값을 가진다.
	*/
	...
	System.out.println(list.remove("마이걸")
			? "삭제성공(마이걸)" : "삭제실패(마이걸)");
	...
	/*
	 전체삭제: 아개 2개의 메서드를 통해 컬렉션에 저장된 전체 데이터를
	 	삭제할 수 있다.
	 */
	list.remove(list);
	list.clear();
