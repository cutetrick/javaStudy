...
//		ArrayList<Student> list2 = new ArrayList<Student>();
		...
		list2.add(st3);
		list2.add(st4); //인덱스 3
		//List는 중복저장이 가능하다. 인덱스로 구분 할 수 있다.
		list2.add(st2);
		...
		for(int i =0; i<list2.size(); i++) {
			/*
			 ...
			 toStirng ()메서드를 오버라이딩 했으므로 인스턴스에 저장된
		...
		/*
		 컬렉션에 지정된 값을 통해 이터레이터 인스턴스를 생성한다.
		 반환할 인스턴스가 있는지 확인 한 후 true이면 next()로 출력한다.
		 */
		...
		//리스트에 저장된 인스턴의 갯수만큼 반복
		for (Student st : list2) {
			/*
			 이름으로 검색시 멤버변수 name은 private으로 선언되었으므로
			 getter를 통해 접근해야한다. 삭제를 위해 입력한 이름과 Student
			 인스턴스에 저장된 name과 일치하는지 확인 한 후 해당 인스턴스를
			 찾을 수 있다.
			 */
