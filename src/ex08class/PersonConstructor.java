...
public class PersonConstructor {
	...
	/*
	 ...
	 '디폴트 생성자'가 컴파일러에 의해 자동으로 삽입된다.
	 지금까지 인스턴스 생성시 new 클래스명()과 같이 사용했떤 문장이 바로 디폴트
	 생성자를 호출하여 것이었다.
	 */
	...
	public PersonConstructor(String _name, int age, String addr) {
		...
		name = _name;
		//둘이 이름이 동일한 경우에는 this를 사용하도록 권고하고 있다.
		this.age = age;
		this.addr = addr;
	...
	void initalize(String name, int age, String addr) {
		/*
		 해당 코드는 생성자 내에서만 사용할 수 있다. 생성자는 인스턴스 생성시
		  ...
		  하지만 멤버메서드는 개발자가 원할 때 언제든 호출이 가능하므로 생성자와는
		  특성이 다른다.
	...
	void showPersonInfo() {
		System.out.printf("%s님의 정보n", this.name);
