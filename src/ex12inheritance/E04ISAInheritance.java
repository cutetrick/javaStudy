...
//컴퓨터: 기본적인 컴퓨터 환경을 제공하여 입력된 내용을 계산
class Computer {
	//멤버변수: 컴퓨터의 소유주
	String owner;
	//생성자
	...
	/*
	 private으로 선언된 멤버메서드를 외부에서 호출 할 수 있드록
	 public으로 지정하여 선언하였다.
...
class NotebookComputer extends Computer {
	...
	public NotebookComputer(String name, int initCharge) {
		/*
		 ...
		 또한 여기서는 부모클래스에 디폴트생성자가 없는 상태이므로 생략하는 즉시 에러가
		 발생한다.
		 */
		super(name);
		battary = initCharge;
...
/*
  테블릿 노트북 : 컴퓨터의 기본 연산기능과, 휴대이용 그리고 전용펜으로
...
class TabletNotebook extends NotebookComputer{ 
    ...
    public void write(String penInfo) {
        ...
        문자열, -1 혹은 1이 반환되면 다른 무자열로 판단하는 String
        클래스에 정의된 메서드 
         */
        ...
        /*
         ...
         이와같이 부모의 멤버메서드를 호출하여 자신의 멤버처럼 사용 할 수 있다.
         단 접근이 가능ㅎ나 접근지정자로 선언되어야 한다.
         super는 생략할 수 있다.
         */
       super.movigCal();
