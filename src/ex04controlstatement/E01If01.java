...
/*
 if문형식 1
 	...
 	- 즉 결과값이 boolean타입으로 rtue/flase를 반환해야 한다.
...
//		}
		...
		/*
		 조건 검사후 실행할 문장이 1개인 경우에는 중괄호를 생략할 수 있다.
		 단, 실행문이 2개이상이라면 한 문장만 if문에 포함되므로 반드시
		 중괄호를 기술해야한다.
		 */
		if(num%2 ==0)
			System.out.println("\n"+num+"은 짝수이다");
		...
		/* 시나리오] 사용자가 입력한 문자가 숫자인지 판단하는 프로그램을 if 문으로 작성하시오.
		 */
		
		/*
		 아스키코드를 알고있을때의 구현방법. 문자 0과 9가 아스키코드 48,, 57에 해당하므로
		 구간내에 있는지 확인할 수 있다.		 */
		...
		/*
		 아스키코드를 모를때 구현하는 방법으로 문자'0'과 '9'는 메모리에
		 저장시 아스키코드로 변환되므로 아래와 같은 조건으로도
		 확인할 수 있다.
		 */
