...
/*
 함수형 인터페이스
 : 람다식을 사용하기 위해서는 인터페이스를 머넞 만들고 람다식으로 구현할
 추상메서드를 선언한다. 이를 람다식을 만들기 위한 '함수형 인터페이스' 라고
 한다. 함수형 인터페이스에는 오직 하나의 추상메서드만 선언할 수 있다.
 */

/*
 @FunctionalInterface: 함수형 인터페이스의 조건을 갖추었는지에 대한
	건사를 컴파일러에게 요청ㅇ하는 어노테이션이다.
 */
	@FunctionalInterface
	interface ISchool4 {
	...
	public static void main(String[] args) {
		/*
		 ...
		 같이 String 을 생략 할 수 있다.
		 */
		ISchool4 sch = (str) ->
			System.out.println("ISchool4를 구현받아 정의한 람다입니다."+str);
			
			/*
			 앞에서 정의한 람다식을 메서드의 인수로 전달한다. 이때 람다식 자체가
			 전달되는것이 아니라 참조값만 전달된다. 참조값을 통해  우리는
			 람다식의 기능을 호출 할 수 있다.
			 */
			execute(sch, "\n람다를 매개변수로 전달합니다.");
	}
