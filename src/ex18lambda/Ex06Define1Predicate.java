...
public class Ex06Define1Predicate {
	...
	public static void main(String[] args) {		
		...
		/*
		 Predicate<T>
		 - 사전적 의미: ~에 근거를 두다.
		 - 매개변수와 boolean 리턴 값이 있는 추상메서드 test()가 정의되어있다.
	...
	/*
	 ...
	 단 해당 컬렉션은 값을 변경 할 수는 없고 참조만 가능하다.
	 */
	List<Human> list1 = Arrays.asList(p1, p2, p3, p4);
	/*
	Predivate<Human> aaa =
	 (Human h) -> h.getGender().equals("남");
	 이와같이 람다식을 저의한 후 참조변수인 aaa를 전달하는것과 동일하게
	...
	public static double avg(Predicate<Human> ph, List<Human> li) {
		...
		int pCount = 0;
		int tCount = 0;
		//List에 저장된 Human 인스턴스의 갯수만큼 반복(확장for문)
		for (Human h :li) {
			//람다식을 통해 각 Human인스턴스의 성별을 판단한다.
			if(ph.test(h)==true) {
				//'남'인 경우에만 변수를 1증가
				pCount++;
				//점수를 합산
