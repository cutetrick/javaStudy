...
/*
  ...
  -인스턴스 생성은 할 수 없지만 그 외 참조변수선언, 상속등의 모든 기능을 사용할
  수 있다.
...
/*
 추상메서드를 포함하지 않은 추상클래스를 상속하면 오버라이딩은 가제사항이
 아닌 선택사항이된다. 즉 필요한 경우에만 오버라이딩 하면 된다.
...
/*
 추상 클래스2 : 추상메서드를 포함한 클래스. 추상메서드를 멤버로 포함한
...
/*
 ...
 인스턴스를 생성할 수 없게 된다. 따라서 인스턴스 생성을 위해서는
 부모의 추상메서드를 자식쪽에서 오버라이딩을 필수로 해야한다.
 */
class AbsClassChild2 extends AbsClass2 {
	/*
	 오버라이딩은 부모쪽의 메서드를 가리고, 새로운 기능을 재정의
	 하는것이므로 자식 클래스는 추상메서드를 포함하지 않는 상태가 된다.
	 따라서 인스턴스 생성을 할 수 있다.
