...
//	 char chStr ='가나다라';
	 ...
	 /*
	  아스키코드: 1byte로 표현할 수 있는 문자로 영문, 순자를 십진수로 정의한 것을
	  말한다.
	   EX) A => 65, a =>97로 표현된다.
	  유니코드 : 1byte로 표현할 수 없는 문자로 영문과 숫자를 제외한 모든
	  문자 (한글, 한자 등)를 2byte로 표현한다. 보통 숫자가 크기떄문에
	  16진수로 표현한다.
	  */
	 //메모리에 65로 저장된다.
	 char ch2 = 'A';
	 ...
	 /*
	  &&(논리 And): 엠퍼센트를 사용하고 양쪽 모두 참일때만 참을 반환한다.
	  	그 외에는 모두 거짓을 반환한다.
