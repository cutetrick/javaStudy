...
public class E01JunsuType {
	...
	public static void main(String[] args) {
		...
		/*
		 * 100/200의 결과는 0이 나오게된다. Java에서는 정수와 정수의 연산결과로 정수가 반환된다. 만약 실수의 결과를
		...
		/*
		 ...
		 - int형보다 작은 자료형을 연산하면CPU는 int형으로 변환하여 계산한다.
	     - CUP는 정수의 연산인 경우 int형에 최적화 되어있기 때문이다.
	     - byte, short형은 게임캐릭터의 움직임이나 음원파일 등을 표현 할 떄 주로 사용된다.
		 */
		...
		/*
		 ...
		 - 하지만 큰 자료형과 작은 자료형을 연산하면 큰 자료형으로 자동 형변환된다.
		  이유는 데이터의 손실(오차)가 있을 수 있기 때문이다.
		 */
		long long1 = 1000, long2 = 2000;
		long result4 = int1 + long1;
		System.out.println("long1 + long1 =" + result4);
		
		result4 = int1 + long2;
		System.out.println("long1 + long2 =" + result4);
		
		/*
		 자바 컴파일러는 정수형 상수를 기본적으로 int형으로 간주하기 때문에 아래 정수를 메모리에
		 로드하는 순간 에러가 발생한다.
		 이때 접미사(L혹은 l)을 붙여서 int형 자료가 아닌 long형의 자료임을 컴파일러에게 알려줘야한다.
		 이와같이 할당을 위해 입력되는 숫자도 자료형을 기반으로 메모리에 로드되는데, 이를 가르켜
		 리터럴(literal)이라고 한다.
		 */
