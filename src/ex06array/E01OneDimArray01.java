...
/*
 배열: 순차적인 메모리의 공간이 할당되는 형태로써 하나가 아니라 여러개의
 변수가 필요한 경우에 사용한다.
 - Java에서는 배열을 인스턴스 형태로 생성하므로 new를 사용한다.
 - 생성된 배열은 Heap(힙) 메모리 공간에 저장되고, 할당된 메모리의 주소값을
 반환한다.
 - 따라서 배열명(배열변수)는 주소값(참조값)을 할당받게된다.
 - 우리는 해당 주소값을 통해 배열에 접근할 수 있다.
 */
public class E01OneDimArray01 {
	...
	public static void main(String[] args) {
		...
		numArr[2]=30;
		//인덱스로 접근하여 할당된 값을 출력
		System.out.println("numArrdml 0번방의 값:" + numArr[0]);
		System.out.println("numArrdml 1번방의 값:" + numArr[1]);
		System.out.println("numArrdml 2번방의 값:" + numArr[2]);
		...
		/*
		 배열을 선언과 동시에 초기화하는 경우 자바 컴파일러는 초기값의
		 갯수를 통해 배열의 크기를 결정할 수 있다. 따라서 이 경우에는
		 크기는 생략하도록 약속되어있다.
		 */
...
//		int[] initArr2 = new int[3] {1, 20, 300};
		...
		/*
		 배열요소의 갯수를 통해 배열을 생성한 후 초기화까지 동시에 진행한다.
		 이 경우 new를 생략할 수 있다.
		 */
