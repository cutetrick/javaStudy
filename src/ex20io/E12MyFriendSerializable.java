...
//   private int numOfFriends;
	/*
	 ...
	 컬렉션은 제네릭을 기반으로 하므로 인스턴스 생성시 저장할 타입을 결정하게되ㅣㄴ다.
...
//      numOfFriends = 0;
   ...
   public void addFriend(int choice) {
      ...
      /*
       List<E>는 자동인덱싱이 지원되므로 인스턴스 추가를 위햐 index변수를 별도로
       사용할 필요가 없다. 단지 add()메서드를 통해 추가하면 된다.
       */
      if(choice == 1) {
   ...
   public void showSimpleData() {
      /*
       확장 for문은 인덱스를 사용하지 않으므로 간단히 인스턴스를
       인출 할 수 있다.
       */
      for(Friend fr :lists) {
    	  fr.showBasicInfo();
   ...
   //프로그램 종료 시 친구의 정보를 직렬화 한다.
	...
	public void readFriendInfo() {
		try {
			...
			/*
			 파일에 친구의 정보가 몇개 저장되었는지 확인 할 수 없으므로
			 무한루프로 구성한다.
			 */
		while(true) {
			/*
			...
			/*
			 예외가 발생하면 catch절로 예외인스턴스가 던져지므로 while루프를 탈출
			 할 수 있다.
			 */
